(() => {
  ...
  const guard = (img, story) => {
    ...
    img.addEventListener('error', () => {
      if (!img.dataset.tdBatchFallback) {
        img.dataset.tdBatchFallback='1'; img.src=story.fallback; return;
      }
      const link=img.closest('a'); img.remove(); link?.classList.add('is-image-missing');
    });
  };
  ...
  const refilter = (archive) => {
    ...
    entries.forEach((entry)=>{const paths=(entry.dataset.paths||'').split(/\s+/).filter(Boolean);entry.hidden=active!=='all'&&!paths.includes(active);});
    archive.querySelectorAll('[data-archive-filter]').forEach((filter)=>{const current=filter.dataset.archiveFilter===active;filter.classList.toggle('is-current',current);if(current)filter.setAttribute('aria-current','page');else filter.removeAttribute('aria-current');});
    const count=archive.querySelector('[data-archive-count]'); const status=archive.querySelector('[data-archive-status]');
  ...
  const apply=()=>{applyArchive();applyHome();}; apply(); requestAnimationFrame(apply); window.addEventListener('load',apply,{once:true});
})();
